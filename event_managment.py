...
def c_data(log):
    ...
    def edit():
        ...
        def c_save():
            # try:
            cur.execute(
                "CREATE TABLE IF NOT EXISTS  college_data (user text,password text,c_name text,c_num text,cmail text)")
            ...
            cur.execute("UPDATE college_data SET password = (?) where c_name = (?)", (password.get(), name.get()))
            con.commit()
            lbl1.config(text="Upated")
            # except:
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS  college_data (user text,password text,c_name text,c_num text,cmail text)")
    cur.execute("select * from college_data")
    c1 = cur.fetchall()
    if c1 == None:
        cur.execute("INSERT INTO college_data values(?,?,?,?,?)",
                    ('', '', 'Feroze Gandhi Institute of Professional College', '', ''))
        con.commit()
    else:
        pass

    root1 = Tk()
    root1.geometry("550x250+400+300")
    root1.title("Basic")
    try:
...
def work(log):
    ...
    def add(wrk):
        ...
        def save():
            try:
                ...
                # cur.execute('select * from my_data order by Edate ASC')
                con.commit()
                ename.set("")
                edate.set("")
                edetails.set("")
    ...
    v_bttn.grid(padx=20, pady=30, row=3, column=1)
    lbl_3 = Label(wrk, text="college details", font=("ariel", 15, "bold"), fg="green",relief="groove")
    lbl_3.grid(padx=40, pady=30, row=3, column=2)
    lbl_3.bind("<Button>", lambda e: c_data(wrk))
...
def admin():
    ...
    def w():
        if username.get() == user1 and password.get() == pss1:
            ...
            lbl_01.config(text="Wrong input!")

    u_bttn = Button(log, width=10, fg="black", text="Login", command=w)
    u_bttn.grid(row=5, column=3)

    # lbl_1=Label(log,text="",font=("ariel",8,"bold"),fg="green")
    # lbl_1.grid(row=6,column=2)
    lbl_01 = Label(log, text="", font=("ariel", 8, "bold"), fg="red")
    lbl_01.grid(row=4, column=3)
